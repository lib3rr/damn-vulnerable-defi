...
contract NaiveReceiverPool is Multicall, IERC3156FlashLender {
    ...
    error RepayFailed(); // unused Error
    error UnsupportedCurrency();
    error CallbackFailed();

    constructor(address _trustedForwarder, address payable _weth, address _feeReceiver) payable { // what it means that a constructor is payable???
        weth = WETH(_weth);
