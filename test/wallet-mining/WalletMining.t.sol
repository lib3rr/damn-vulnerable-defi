...
contract WalletMiningChallenge is Test {
    ...
    function test_walletMining() public checkSolvedByPlayer {
        console.log(authorizer.needsInit());

        // copy pasted writeup from the internet, the nonce bruteforcing does not work :/
        ...
        for (uint256 i = 0; i < 1; i++) {
            SafeProxy pp = proxyFactory.createProxyWithNonce(address(singletonCopy), initializer, i);
            if (address(pp) == USER_DEPOSIT_ADDRESS) {
                console.log(i);
                nonce = i;
                break;
            }
